...
void set_mpu(void)
{
	...
	//??????
	REG(MPU_BASE + MPU_RBAR_OFFSET) = MPU_RBAR_VALUE((uint32_t)0x00000000, 0);
	...
	//??????
	REG(MPU_BASE + MPU_RBAR_OFFSET) = MPU_RBAR_VALUE((uint32_t)0x20000000, 1);
	...
	//disable region 4 ~ 7
	for(int region = 4; region < 8; region++)//for (??????)
	{
		//??????
        REG(MPU_BASE + MPU_RBAR_OFFSET) = MPU_RBAR_VALUE((uint32_t)0x00000000, region);
        REG(MPU_BASE + MPU_RASR_OFFSET) = 0; //bit 0 means MPU_RASR is disable
	}
